...
export class BlobAnimation {
  ...
  public init({
    ...
    this.flubberAnimations.forEach((animation, index) =>
      animation.start(duration + (layerDelay || 0) * index),
    );
  }
  ...
  private generateFlubberAnimations() {
    return this.selectors.map(
      (selectorArr) => new FlubberAnimation(...selectorArr),
    );
  }
  ...
  private get selectors(): string[][] {
    ...
    for (let layer = 1; layer <= this.layers; layer++)
      selectors.push(
        this.stepSelectors.map(
          (s) => `${this.blobSelector} .layer-${layer} ${s}`,
        ),
      );
