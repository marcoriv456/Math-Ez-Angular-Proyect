...
export class RootValidator extends TermValidator {
  ...
  protected override getValidationMessages(): TermValidationMessage[] {
    ...
    if (!isNaN(this.radicandValue)) this.validateRadicandValue(messages);
  ...
  private invalidateNegativeRadicandAndOddIndex(
    messages: TermValidationMessage[],
  ) {
    if (
      this.radicandValue < 0 &&
      ((this.indexValue && this.indexValue % 2 == 0) ||
        this.indexStringValue == '')
    )
      messages.push({
        message:
          'La raiz de un numero negativo con indice par dara un numero complejo.',
        type: 'fully-invalid',
      });
  }
