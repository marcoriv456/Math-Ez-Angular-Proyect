...
export class LogarithmValidator extends TermValidator {
  ...
  constructor(term: Term) {
    ...
    if (term.type !== 'function' || term.functionName !== 'log')
      throw new UnexpectedTermTypeError(
        'log function',
        `${term.type == 'function' ? term.functionName : ''} ${term.type}`,
      );
  ...
  protected override getValidationMessages(): TermValidationMessage[] {
    ...
    if (!isNaN(this.baseValue)) this.validateBase(messages);
  ...
  private invalidateBaseLessOrEqualToOne(messages: TermValidationMessage[]) {
    if (this.baseValue <= 1 && this.baseStringValue !== '')
      messages.push({
        message:
          'La base de un logaritmo debe de ser un numero positivo distinto de 1.',
        type: 'fully-invalid',
      });
  }
  ...
  private invalidateArgumentLessThanZero(messages: TermValidationMessage[]) {
    if (this.argumentValue < 0)
      messages.push({
        message:
          'Los logarimos con argumentos negativos devuelven numeros complejos.',
        type: 'fully-invalid',
      });
  }
  ...
  private warnBaseEqualToTen(messageList: TermValidationMessage[]) {
    if (this.baseValue == 10)
      messageList.push({
        ...
        message:
          'Aun si no se especifica, 10 es el valor por defecto de la base de un logaritmo.',
      });
  }
}
