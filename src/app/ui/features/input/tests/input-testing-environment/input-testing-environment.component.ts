...
@Component({
  ...
  standalone: true,
  imports: [InputModule],
  providers: [ExperimentalVariableProviderService],
})
export class InputTestingEnvironmentComponent implements AfterViewInit {
  ...
  ngAfterViewInit() {
    this.inputComponent.variableHandler.setVariableProvider(
      this.expVariableProvider,
    );
    this.inputComponent.setTerms(
      ...
      {
        type: 'fraction',
        numeratorChildren: [{ char: '1', type: 'char' }],
        denominatorChildren: [{ char: '8', type: 'char' }],
      },
      { char: ' ', type: 'char' },
      { char: '8', type: 'char' },
      {
        exponentChildren: [{ char: '2', type: 'char' }],
        type: 'exponent',
      },
    );
  }
