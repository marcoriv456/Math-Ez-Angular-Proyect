...
@Component({
  template: ` <test [inputTermValidation]="TestValidator"></test> `,
})
  ...
  providers: [
    {
      ...
      useExisting: forwardRef(() => TestingMathElement),
    },
  ],
})
...
describe('InputTermValidationDirective', () => {
  ...
  beforeEach(async () => {
    await TestBed.configureTestingModule({
      ...
      providers: [InputEventBusService],
    }).compileComponents();
    ...
    const debugEl = fixture.debugElement.query(
      By.directive(InputTermValidationDirective),
    );
  ...
  describe(`Requesting warnings: `, () => {
    it(`Requests to show a warning if the mouse is over it.`, () => {
      ...
      expect(eventBus.emit).toHaveBeenCalledWith(
        expect.any(WarningShowRequestEvent),
      );
    });

    it(`Requests to show a warning with the correct location data`, () => {
      const expectedMessages: TermValidationMessage[] = [
    ...
    it(`Requests to hide a warning if the mouse leaves it.`, () => {
      ...
      expect(eventBus.emit).toHaveBeenCalledWith(
        expect.any(WarningHideRequestEvent),
      );
    });
  });
});
