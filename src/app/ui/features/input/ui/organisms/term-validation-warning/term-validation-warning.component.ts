...
export class TermValidationWarningComponent implements OnInit {
  ...
  ngOnInit() {
    this.eventBus
      .on(WarningShowRequestEvent)
      .subscribe((event) => this.showWarning(event.renderData));
    this.eventBus
      .on(WarningHideRequestEvent)
      .subscribe(() => this.hideWarning());
  }

  private showWarning({ messages, position }: TermValidationWarningRenderData) {
    this.showingWarning = true;

    this.warningStyleData.left =
      position.x - this.ref.getBoundingClientRect().left + 'px';
    this.warningStyleData.top =
      position.y - this.ref.getBoundingClientRect().top + 'px';
