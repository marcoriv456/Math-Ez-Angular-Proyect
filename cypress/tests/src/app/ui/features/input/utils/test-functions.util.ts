...
export const getPosition = (element: JQuery<HTMLElement>) =>
  Math.floor(element.get()[0].getBoundingClientRect().left);

export const getFrontPosition = (element: JQuery<HTMLElement>) =>
  Math.floor(getPosition(element) + (element.width() || 0));

export const getCaretPosition = () =>
  cy.get(`.caret-container`).then(getPosition);

export const getElementPosition = (selector: string) =>
  cy.get(selector).then(getPosition);
export const getElementFrontPosition = (selector: string) =>
  cy.get(selector).then(getFrontPosition);

export const getLetterPosition = (letter: string) =>
  cy.contains(`char`, letter).then(getPosition);
export const getLetterFrontPosition = (letter: string) =>
  cy.contains(`char`, letter).then(getFrontPosition);
...
export const getHeight = (el: JQuery<HTMLElement>) =>
  Math.floor(el.outerHeight() || 0);
