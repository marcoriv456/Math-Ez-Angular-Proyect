...
} from './utils/test-functions.util';
import {
  expSelector,
...
describe(`Input component`, () => {
  ...
  describe('Term validation:', () => {
    ...
    const expectWarningToBeDisplayed = (invalidElementSelector: string) => {
      cy.get(invalidElementSelector).trigger('mouseover');

      cy.get('.warning-container').should('exist').and('be.visible');
    };
  ...
  runEditableElementSuite(
    `Main`,
    `{ctrl}`,
    `[data-cy-root] editable-term-container`,
  );
});
